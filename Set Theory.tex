...
\end{exercise}
\begin{proof}
	Thanks to \exerciseref{3.5.10}, we can assume that $(A^B)^C$ and $A^{B\times C}$ take these forms:
	\begin{align*}
		\{C, A^B, G_1\} &\in (A^B)^C \\
		\{B \times C, A, G_2\} &\in A^{B\times C}
	\end{align*}
	, where elements in $G_1$ and $G_2$ take these forms, assuming $a \in A$, $b \in B$, and $c \in C$:
	\begin{align*}
		(c, \{B, A, G_3\}) &\in G_1 \\
		((b,c), a) &\in G_2
	\end{align*}
	, where elements in $G_3$ take this form:
	\[
		(b,a) \in G_3
	\]
	
	As the domains and codomains don't change, it suffices to construct a bijection between the set of all $G_1$'s and all $G_2$'s.
	
	We first define some auxiliary functions. Define \textcolor{red}{TBD.}
	
	First, define a bijection, $g_1$ that, using the axiom of replacement, maps each $G_2$ to a set $g_1(G_2)$, in which each element $((c,b),a)$ is obtained from replacing some element $((b,c),a) \in G_2$. Because of \exerciseref{3.6.5}, this is a bijection. Because any $G_2$ satisfies the vertical line test, $g_1(G_2)$ also satisfies the vertical line test.
