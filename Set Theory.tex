...
(f)
	...
	We also prove it when $\#X = 1$, for use in the inductive step. In this case, $X = \{x\}$, and define a bijection from $Y$ to $Y^X$ by $y \mapsto \{X,Y,\{(x,y)\}\}$, where $\{(x,y)\}$ is the graph of some function from $X \to Y$. Thus, $\#(Y^X) = \#Y = (\#Y)^1$.
	...
	Because of \exerciseref{3.5.10}, we can treat $Y^{X \setminus \{x\}}$ as containing such elements: $\{X \setminus \{x\}, Y, G\}$, where $G$ is the graph of some function from $X \setminus \{x\}$ to $Y$.
...
\fbox{\em Surjectivity} Let $(y,x)$ be any element in $Y \times X$, then $y \in Y$ and $x \in X$. Thus, $(x,y) \in X \times Y$, and by definition $f((x,y)) = (y,x)$.

\fbox{\em Injectivity} Let $(x,y) \ne (x',y')$ be any two elements in $X \times Y$. By definition, 
\[
	f((x,y)) = (y,x) \ne (y',x') = f((x',y'))
\]
...
\begin{proof}
	...
	If $B$ is empty, then $A^B$ only contains an (unique, thanks to Why~\ref{why.empty.func.is.unique}) empty function $f: \emptyset \to A$. But $B \times C$ is also empty, thanks to \exerciseref{3.5.8}, so $A^{B \times C}$ also only contains the empty function $f: \emptyset \to A$. Hence, the bijection is obvious.
	
	If $C$ is empty, then we have $(A^B)^C = \{f: \emptyset \to A^B\}$ and $A^{B \times C} = \{g: \emptyset \to A\}$. The bijection is $f \mapsto g$.
		
	Now, suppose none of $A,B,C$ is empty. Thanks to \exerciseref{3.6.4} (f), both of $(A^B)^C$ and $A^{B\times C}$ are also non-empty. Let $f$ be any function in $(A^B)^C$. Define a function $g$ in $A^{B \times C}$ this way:
	\[
	...
	\fbox{\em Surjectivity} Let $g$ be any function in $A^{B \times C}$. By \exerciseref{3.5.10}, $g$ has a graph $G$, in which elements take the form $((b,c), a)$. 
...
Now we show that $(a^b)^c = a^{b \times c}$ for any natural numbers $a,b,c$. Define $A := \{1 \le i \le a\}$, $B := \{1 \le i \le b\}$, and $C := \{1 \le i \le c\}$. By \exerciseref{3.6.4} (f), twice, we see that $(A^B)^C$ is finite and $\#(A^B)^C = (a^b)^c$.

By \exerciseref{3.6.4} (e) first and (f) second, we see that $A^{B \times C}$ is finite and $\#A^{B \times C} = a^{b \times c}$. By what we have proven, $\#(A^B)^C = \#A^{B \times C}$. Hence, $(a^b)^c = a^{b \times c}$.
...
\begin{proof}
	...
	Now, suppose that none of $A,B,C$ is empty. Thanks to \exerciseref{3.6.4} (b), (e), and~(f), both of $A^B \times A^C$ and $A^{B \cup C}$ are non-empty.	
	...
	\fbox{\em Surjectivity} Let $h$ be any function in $A^{B \cup C}$. Define $f: B \to A, f(b) := h(b)$ and $g: C \to A, f(c) := h(c)$. We need to show that $\phi((f,g)) = h$. Let's for now call $h' := \phi((f,g))$.
...
By \exerciseref{3.6.4} (f) first and (e) second, we see that $A^B \times A^C$ is finite and $\#A^B \times A^C$ = $a^b \times a^c$.
By \exerciseref{3.6.4} (b) first and (f) second, we see that $A^{B \cup C}$ is finite and $\#A^{B \cup C} = a^{b + c}$. 
...
\myifbox Suppose there is an injection $f$ from $A$ to $B$. Then, $f: A \to f(A)$ is a bijection, and $\#A = \#f(A)$. Because $f(A) \subseteq B$, by \exerciseref{3.6.4} (c), $\#f(A) \le \#B$, as desired. Note that the fact that $B$ is finite is necessary to conclude this.

\myoifbox Suppose that $a \le b$. By the definition of cardinality, there is a bijection $f: \{1 \le i \le a\} \to A$, and a bijection $g: \{1 \le i \le b\} \to B$. Because $a \le b$, the function $h: i \mapsto i$ is an injection from $\{1 \le i \le a\} \to \{1 \le i \le b\}$. Consider the function $g \circ h \circ f^{-1}$. It is a function from $A$ to $B$ by definition. And thanks to \exerciseref{3.3.2}, it is injective, too.
\end{proof}
...
\end{exercise}
\begin{proof}
	By \exerciseref{3.6.4} (b), $X \cup Y$ is finite. By definition, $X \cap Y \subseteq X$. By \exerciseref{3.6.4} (c), $X \cap Y$ is finite.
	
	Now we show that $\#(X \cup Y) + \#(X \cap Y) = \#X + \#Y$.
	
	Define $X' := X \setminus (X \cap Y)$. We have $X' \cap Y = \emptyset$. By \exerciseref{3.6.4} (b), we have $\#(X' \cup Y) = \#X' + \#Y$.
	...
	\begin{enumerate}
		...
		\item If $a \in X$ but not both, then $a \in X \setminus Y$. By \exerciseref{3.1.12} (ii) (a special case of it where $A = A'$), we have $X \setminus Y \subseteq X \setminus (X \cap Y)$. Hence, $a \in X \setminus (X \cap Y) = X'$, as desired.
	\end{enumerate}

	Therefore, we now know that $\#(X \cup Y) = \#X' + \#Y$. We show that $\#X' + \#(X \cap Y) = \#X$. Observing that $X' \cap (X \cap Y) = \emptyset$, by definition. Then, we can use \exerciseref{3.6.4} (b) on it to conclude that $\#X = \#(X' \cup (X \cap Y)) = \#X' + \#(X \cap Y)$, as desired.
...
\begin{proof}
	...
	By the Axiom of Union, $\bigcup_{1 \le i \le \successor{n}}A_i = \bigcup_{1 \le i \le n}A_i \cup A_{\successor{n}}$. By \exerciseref{3.6.4} (b), we have 
	\begin{align*}
...
vertsub{f}{N_n}$, and is at least injective. Because $f$ is a bijection, by \exerciseref{3.4.3} (b), we know that $f(N_n) = f(N_{\successor{n}} \setminus \{\successor{n}\}) = f(N_{\successor{n}}) \setminus f(\{\successor{n}\})$. Because $f$ is a bijection, $f(N_{\successor{n}}) \setminus f(\{\successor{n}\}) = N_{\successor{n}} \setminus \{successor{n}\} = N_n$, as desired.

		If $f^{-1}(\successor{n}) \ne \successor{n}$, then let $m = f^{-1}(\successor{n})$. We have $N_n \setminus \{m\}$ having a cardinality of $n-1$ (informally, as in Lemma~3.6.9). As $f$ is injective, according to \exerciseref{3.6.4} (d), $\#f(N_n \setminus \{m\})$, too. The injectivity also grants that $f(\successor{n}) \notin f(N_n)$, so we have 
		\[
			\#f(N_n \setminus \{m\} \cup \{\successor{n}\}) = n = \#N_n
		\]
		By \exerciseref{3.4.3}, we have $f(N_n \setminus \{m\} \cup \{\successor{n}\}) = f(N_n \setminus \{m\}) \cup f(\{\successor{n}\})$. By the uniqueness of $m$, both sides of the $\cup$ are subsets of $N_n$. Hence their union is also a subset of $N_n$. Now consider \exerciseref{3.6.4} (c), which states that, given $Y \subseteq X$, where $X$ is finite, then
		\[
			Y \ne X \to \#Y < \#X
		\]
...
Now we can prove this exercise's statements.
\begin{proof}\leavevmode
\begin{enumerate}
	\item 
		By \exerciseref{3.6.4} (f), the set of all functions from $N_n$ to itself is finite. Hence, $S_n$, being a subset of it, is also finite, by (c).
		...
		\exerciseref{3.6.4} (e), we have $\#(S_n \times N_{\successor{n}}) = \#S_n \times \successor{n}$, as desired.
